...
public class Elgamal_KeySet {
	...
	public void Elgamal_KeySet(Elgamal_Parameters par) {
		...
		do {
			...
			if (p.compareTo(g) == 1 && g.modPow(p_prime, p).equals(BigInteger.ONE)
					&& !g.modPow(new BigInteger("2"), p).equals(BigInteger.ONE)) {
				found = true;
			}
		} while (!found);
	...
	public static Elgamal_KeySet newKset(String[] params) {
		Elgamal_KeySet kset = new Elgamal_KeySet(Elgamal_PublicKey.fromJson(params[0]),
				Elgamal_SecretKey.fromJson(params[1]), Integer.parseInt(params[3]));
	...
	public static Elgamal_KeySet newPublicKset(String[] params) {
		Elgamal_KeySet kset = new Elgamal_KeySet(Elgamal_PublicKey.fromJson(params[0]), new Elgamal_SecretKey(),
