...
public class Elgamal {
		...
		while (!signingDone) {
			...
			for (int i = 0; i < pt.getPt().length; i++) {
				...
				if (pt.getPt()[i].compareTo(pmin1) == 1) {
					// System.out.println("mod "+ modulo+" bytes
					// "+modulo.bitCount()+" "+pt.getPt()[i]);
	...
	public Elgamal_CipherText sign(String s) {
		...
		// String(chuncks[chuncks.length-1].toByteArray()));
		// encrypt
	...
	public Elgamal_CipherText encrypt(Elgamal_PlainText pt) {
		...
		for (int i = 0; i < pt.getPt().length; i++) {
			...
			// System.out.println("mod "+ modulo+" bytes "+modulo.bitCount()+"
			// "+pt.getPt()[i]);
	...
	public Elgamal_PlainText decrypt(Elgamal_CipherText ct) {
		...
		// BigInteger s=ct.getGr().modPow(kset.getSk().getX(),
		// kset.getPk().getP());
		// BigInteger
		// decrypt=ct.getCt().multiply(s.modInverse(kset.getPk().getP())).mod(kset.getPk().getP());
	...
	public String Elgamal_CtToString(Elgamal_CipherText pt) {
		String res = "";
		for (int i = 0; i < pt.getCt().length; i++) {
			res += new String(pt.getCt()[i].toByteArray());
		}
		return res;
	}
	...
	/*
	 * This is the Extended Euclidean Algorithm and is used as part of finding
	 * the modular inverse. Implemented from pseudocode found here first
